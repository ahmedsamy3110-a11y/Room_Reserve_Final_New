...
async function handleApi(req, res) {
  ...
  try {
    ...
    const adminBookingDeleteMatch = url.pathname.match(/^\/api\/admin\/bookings\/([^/]+)$/); if (adminBookingDeleteMatch && req.method === 'DELETE') { const admin = await requireAdmin(req, res); if (!admin) return; const id = decodeURIComponent(adminBookingDeleteMatch[1]); const booking = hasPostgres ? (await query('SELECT * FROM bookings WHERE id=$1', [id]))[0] : memory.bookings.find(b=>b.id===id); if (!booking) return jsonResponse(res, 404, { message: 'Booking not found.' }); if (hasPostgres) await query('DELETE FROM bookings WHERE id=$1', [id]); else memory.bookings = memory.bookings.filter(b=>b.id !== id); return jsonResponse(res, 200, { message: 'Booking deleted successfully.' }); }
